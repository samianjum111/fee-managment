...
def gym_edit_attendance(request, schema_name, attendance_id):
    ...
    with schema_context(schema_name):
        ...
        if request.method == 'GET':
            ...
            if form.is_valid():
                form.save()
                # Recalculate duration if check_out changed
...
def gym_checkin_api(request):
    ...
    with schema_context(schema_name):
        ...
        customer = get_object_or_404(GymCustomer, id=customer_id)
        today = timezone.localdate()
        # Check if customer has paid subscription for current month (or allow partial if setting enabled)
        # For simplicity, we require status='paid' – but can be changed to allow 'partial' if needed.
        has_paid = customer.subscriptions.filter(month=today.month, year=today.year, status='paid').exists()
        if not has_paid:
            return JsonResponse({"error": "No active paid subscription for current month"}, status=403)
...
def gym_attendance_data_api(request, schema_name):
    ...
    with schema_context(schema_name):
        ...
        active_qs = GymAttendance.objects.filter(date=today, check_out__isnull=True).select_related('customer').order_by('-check_in')
        active = [{
            'id': a.id,
        ...
        history_qs = GymAttendance.objects.filter(date=today, check_out__isnull=False).select_related('customer').order_by('-check_out')
        history = []
        for a in history_qs:
            duration = ''
            if hasattr(a, 'duration_minutes') and a.duration_minutes:
        ...
        # Stats
        total_checkins = GymAttendance.objects.filter(date=today).count()
        unique_customers = GymAttendance.objects.filter(date=today).values('customer').distinct().count()
        return JsonResponse({
            'active': active,
            'history': history,
            'today_stats': {
...
def gym_eligible_customers_api(request, schema_name):
    ...
    with schema_context(schema_name):
        ...
        today = timezone.localdate()
        # Customers with a paid subscription this month AND not already checked in today
        customers = GymCustomer.objects.filter(
            Q(subscriptions__month=today.month, subscriptions__year=today.year, subscriptions__status='paid') &
            ~Q(attendances__date=today)
        ).distinct().only('id', 'name', 'phone')
...
def gym_export_attendance_api(request, schema_name):
    ...
    with schema_context(schema_name):
        ...
        today = timezone.localdate()
        attendances = GymAttendance.objects.filter(date=today).select_related('customer').order_by('-check_in')
